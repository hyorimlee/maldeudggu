...
class CustomESNet(nn.Module):
    def __init__(self, num_classes, giveEncoder=None):
        ...
        self.in_features = nn.Conv2d(in_channels=1, out_channels=3, kernel_size=(1,1))
        self.classifier = nn.Linear(6*128*504, num_classes)

		
    def forward(self, rgb_input, only_encoder=False):
        output = self.in_features(rgb_input)
        ...
        if only_encoder:
            ...
            return self.classifier(output.view(-1, 6*128*504))